...
from colsemantics.vocabularies import load_vocabularies
...
def test_custom_vocabulary_is_scoped(tmp_path):
    ...
    try:
        assert inferir_semantica("cost_bucket")["semantica"] == "Financeiro / Custo"
    finally:
        reset_context(token)
    assert inferir_semantica("cost_bucket")["semantica"] != "Financeiro / Custo"
...
def test_abreviatura_especulativa_de_duas_letras_nao_e_tentada():
    ...
    assert (
        inferir_semantica("nm")[  # ainda funciona: curado, não especulativo
            "papel"
        ]
